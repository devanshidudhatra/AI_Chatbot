...
from llama_index import SimpleDirectoryReader    #this will take all files from folder and embed them in one code


from langchain_google_genai import ChatGoogleGenerativeAI     #this is to store the previous que
from langchain.chains import ConversationChain                #chain of conversation b/w human and ai
from langchain.memory import ConversationBufferMemory         #store the text we give and then answer
from langchain_core.prompts import ChatPromptTemplate         #give mast prompt


st.set_page_config(page_title="Chat with Document", page_icon="📚")


@st.cache_resource(show_spinner=False)        #store all this in cache.. so that if error is there then streamlit doesn't run again and again
def load_data():
   with st.spinner(text="Loading and indexing the Documents – hang tight! This should take 1-2 minutes."):


       llm = Gemini(api_key="AIzaSyBFDl8NxSiQy0Jr6NDKHdoqeTWNdq9bEqs")
       embed_model = GeminiEmbedding(api_key="AIzaSyBFDl8NxSiQy0Jr6NDKHdoqeTWNdq9bEqs")  # Fix typo
...
if "chat_engine" not in st.session_state.keys():
   ...
   llm = ChatGoogleGenerativeAI(model="gemini-pro", google_api_key="AIzaSyBFDl8NxSiQy0Jr6NDKHdoqeTWNdq9bEqs", temperature=0)
